...
def convert(input_filename, output_filename, from_format=None, to_format=None):
    """Wrap ASE functions to read structure from file and write new format"""
    try:
        structure = ase.io.read(input_filename, format=from_format)
    except:
        raise Exception(
            "Trouble reading input file {0}".format(input_filename))

    structure.write(output_filename, format=to_format)


def main():
    """Convert from one atomistic structure format to another using ASE"""
